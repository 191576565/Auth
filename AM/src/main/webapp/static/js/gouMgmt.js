...
function onEdit(uuid, content) {
	...
	$.getJSON("gouMgmt/loadEditPara?uuid=" + uuid, function(content) {
		...
		$.getJSON("gouMgmt/getUrlDesc?type=" + start, function(content) {
			content.forEach(function(e2) {
				if (start2 != e2.url_uuid) {
					$("#dictcode").append("<option value='" + e2.url_uuid + "'>" + e2.req_url_desc + "</option>");
				}
			})
		});
	});
				...
				onNodeChecked: function(event, data) {
					if (data.nodes != null) {
						...
						for (var i = 0; i < arrayInfo.length; i++) {
							$('#user_in_div').treeview('checkNode', [arrayInfo[i].nodeId, {
								silent: true
							}]);
...
function dosome(num) {
	if (num == 1) {
		$('#user_in_div').treeview('checkAll', {
			silent: true
		}); //全选
	} else if (num == 2) {
		$('#user_in_div').treeview('uncheckAll', {
			silent: true
		}); //取消全选
	} else if (num == 3) {
		$('#user_in_div').treeview('collapseAll', {
			silent: true
		}); //折叠
	} else if (num == 4) {
		$('#user_in_div').treeview('expandAll', {
			levels: 2,
			silent: true
		}); //展开所有二级节点
	}
